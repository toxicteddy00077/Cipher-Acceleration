...
// ============================================================
// Key Schedule (CPU-side, compatible with CPU implementation)
// ============================================================
...
static const uint8_t RCON_HOST[15] = {
...
static void cpu_key_schedule_expand(const uint8_t* key, uint8_t* expandedKey) {
    ...
    for (int i = 8; i < 60; ++i) {
        ...
        if (i % 8 == 0) {
            ...
            for (int j = 0; j < 4; ++j) {
                temp[j] = SBOX_HOST[temp[j]];
            }

            temp[0] ^= RCON_HOST[(i / 8) - 1];
        } else if (i % 8 == 4) {
            for (int j = 0; j < 4; ++j) {
                temp[j] = SBOX_HOST[temp[j]];
            }
...
// ============================================================
// GPU Device Kernels (Forward declarations)
// ============================================================
...
// GPU device pointers for tables
...
void initialize_gpu() {
    ...
    // Load S-boxes to constant memory
    cuda_copy_to_constant("d_SBOX", SBOX, sizeof(SBOX));
    cuda_copy_to_constant("d_INV_SBOX", INV_SBOX, sizeof(INV_SBOX));
    cuda_copy_to_constant("d_RCON", RCON_HOST, sizeof(RCON_HOST));
}

void cleanup_gpu() {
    if (d_expandedKey != nullptr) {
